-- Handle checkout.session.expired: an abandoned Checkout Session (never
-- completed within its expiration window) otherwise leaves its payments row
-- stuck at 'pending' forever, with no cleanup path. Mirrors the existing
-- payment_intent.payment_failed handling, matched by checkout session id
-- since an expired session never reaches a PaymentIntent.
