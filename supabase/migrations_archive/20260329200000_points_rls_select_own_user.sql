-- public.points: authenticated users may SELECT only rows where user_id is their app user (public.users.id).
-- Uses SECURITY DEFINER helper so the policy does not depend on SELECT policies on public.users.
